...
class SanPhamController extends Controller
{
    ...
    {
        $data     = $request->all();
        $san_pham_id = SanPham::find($data['id']);
        $san_pham_id = $request->$san_pham_id;
        $comment = Comment::where('comment_san_pham_id', $san_pham_id)->get();
        $output = '';
        foreach ($comment as $key => $comm) {
           ...
           <div class="col-md-10">
               <p style="font-size: 20px; font-weight: 600;">@' . $comm->comment_name . '</p>
               <p style="font-size: 15px; font-weight: 400;">@' . $comm->comment_date . '</p>
               <p>
                   ' . $comm->comment . '
               </p>
           </div>
